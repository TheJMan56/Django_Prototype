...
def newUser(request):
    userID = int(uuid.uuid4().int & (1 << 31) - 1)
...
def login(request):
    if request.method == 'POST':
        ...
        # Check if username and password match an entry in the Users table
        with connection.cursor() as cursor:
            cursor.execute('SELECT * FROM Users WHERE userName=%s AND loginPassword=%s', [username, password])
            user = cursor.fetchone()
            
            if user is not None:
                return shop(request)
            else:
                # Display error message if login credentials don't match
